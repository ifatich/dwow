...
import { importSprint185 } from "./import-sprint-185";
...
async function scrapeSprint185() {
  ...
  // Parse CSV
  console.log("\n🔍 2. Melakukan parsing dan filtering fokus Sprint 185...");
  ...
  // Filter ONLY Sprint 185
  const sprint185Rows = dataRows.filter((r) => r[spIdx] === "185");
  console.log(`  ✓ Total baris dalam spreadsheet       : ${dataRows.length}`);
  console.log(`  ✓ Total baris fokus Sprint 185        : ${sprint185Rows.length}`);

  // Write focused Sprint 185 CSV
  const s185CSV = toCSV([header, ...sprint185Rows]);
  fs.writeFileSync("data-scrape.csv", s185CSV, "utf-8");
  console.log("  ✓ File data-scrape.csv berhasil diperbarui dengan data murni Sprint 185 saja.");

  // Also save a dedicated copy
  fs.writeFileSync("data-sprint-185.csv", s185CSV, "utf-8");
  console.log("  ✓ File data-sprint-185.csv berhasil disimpan.");

  // 3. Trigger Import into Database
  console.log("\n⚡ 3. Menjalankan import data Sprint 185 ke database taskforge.db...");
  await importSprint185();

  console.log("\n✅ Selesai! Scrap ulang dan import Sprint 185 berhasil 100%.");
}

if (process.argv[1] && process.argv[1].includes("scrape-sprint-185")) {
  scrapeSprint185().catch((err) => {
    console.error("❌ Error scraping Sprint 185:", err);
    process.exit(1);
  });
}

export { scrapeSprint185 };
