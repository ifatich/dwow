...
async function importSprint186() {
  console.log("🚀 Memulai import data Sprint 186 dari Google Sheets (CSV):");
  ...
  // 2. Insert Users
  console.log("2. Menyimpan data pengguna (3 Lead: Arif, Cheppy, Ganda & 18 Staf)...");
  ...
  // Filter Sprint 186 rows
  let totalRawS186 = 0;
  let excludedTentativeZero = 0;
  let excludedBiweekly = 0;
  let excludedSekper = 0;
  let excludedInvalidDetail = 0;

  const validS186Rows: string[][] = [];

  for (const r of rows) {
    if (r[spIdx] !== "186") continue;
    totalRawS186++;
    ...
    validS186Rows.push(r);
  }

  console.log(`  ✓ Total baris Sprint 186 mentah                   : ${totalRawS186}`);
  console.log(`  ✓ Baris tentative 0 jam di-exclude                : ${excludedTentativeZero}`);
  if (excludedInvalidDetail > 0) {
    console.log(`  ✓ Baris tidak valid / detail kosong di-exclude    : ${excludedInvalidDetail}`);
  }
  console.log(`  ✓ Total baris valid yang diimpor                  : ${validS186Rows.length}`);

  // 4. Grouping: Kategori Project (Project) -> Project (Task) -> Task Detail (Subtask)
  const categoryMap = new Map<string, Map<string, string[][]>>();
  for (const r of validS186Rows) {
  ...
  const masterCategories = db.prepare(
  ...
  for (const [categoryName, taskMap] of categoryMap.entries()) {
    ...
    insertProjectStmt.run(
      projectId,
      categoryName,
      `Kategori Project: ${categoryName} — Sprint 186`,
      "Sprint 186",
      leadId,
    ...
    for (const [projectName, subtaskRows] of taskMap.entries()) {
      ...
      const ticketId = `S186-${catCode}-${String(taskCounter).padStart(2, "0")}`;
      ...
      // Level 3: Subtask (Task Detail) — Group by Title + Bobot so co-assignees share 1 card
      ...
      for (const [, rowsInSubtask] of subtaskGroupMap.entries()) {
        const firstRow = rowsInSubtask[0];
        const subtaskId = crypto.randomUUID();
        const rawSubtaskTitle = firstRow[tIdx] || projectName;
        const subtaskTitle = rawSubtaskTitle.replace(/\r?\n|\r/g, " ").replace(/\s+/g, " ").trim();
        const rawBobot = (firstRow[hIdx] || "").trim().replace(",", ".");
        const workload = isNaN(parseFloat(rawBobot)) ? 0 : parseFloat(rawBobot);

        const statusRaw = (firstRow[stIdx] || "to_do").toLowerCase();
        const isDone = statusRaw === "done";
        const subtaskStatus = isDone ? "done" : "to_do";

        const descParts: string[] = [];
        if (firstRow[pengIdx] && firstRow[pengIdx].trim()) descParts.push(`Pengerjaan: ${firstRow[pengIdx].trim()}`);
        if (firstRow[fIdx] && firstRow[fIdx].trim()) descParts.push(`Fitur: ${firstRow[fIdx].trim()}`);
        if (firstRow[nIdx] && firstRow[nIdx].trim()) descParts.push(`Notes: ${firstRow[nIdx].trim()}`);
        ...
        insertSubtaskStmt.run(
          ...
          subtaskTitle,
          description,
          firstRow[fIdx] ? `Fitur: ${firstRow[fIdx].trim()}` : undefined,
          isDone ? 1 : 0,
          subtaskStatus,
          workload,
          now,
        ...
        // Assign contributors (First row is owner, subsequent rows are co-assignees)
        ...
        const primaryOwner = assigneeList[0]?.name || "Staff";
        const coAssigneesText =
          assigneeList.length > 1
            ? `, Co-Assignees: ${assigneeList.slice(1).map((a) => a.name).join(", ")}`
            : "";
        ...
        insertAssignmentHistoryStmt.run(
          ...
          "System (Import Sprint 186)",
          "added",
          `Penugasan subtask (Pemilik: ${primaryOwner}${coAssigneesText})`,
          now
        );
  ...
  console.log(`\n🎉 SELESAI! Data Sprint 186 berhasil diimpor:`);
...
export { importSprint186 };

if (process.argv[1] && process.argv[1].includes("import-sprint-186")) {
  importSprint186().catch((err) => {
    console.error("❌ Error importing Sprint 186:", err);
    process.exit(1);
  });
}
